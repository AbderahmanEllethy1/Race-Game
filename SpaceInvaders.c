...
int flag = 0 , score = 0, arr[3] = {15, 30, 45};
unsigned long j = 2, z = 1;
...
int main(void){
  ...
  Nokia5110_ClearBuffer();
	...
	//	Nokia5110_PrintBMP(hagez0X++,arr[i], SmallEnemy10PointA, 0);
		Nokia5110_PrintBMP(border++, arr[j], enemy, 0);
		if( border >= 62 && j == 2 )
		{
			j = 1 ; 
			flag = 1 ;
		}
		else if(border >= 62 && j == 0 )
		{
			border = 3 ;
			j = 2 ; 
			flag = 0 ;
		...
		if( !(GPIO_PORTF_DATA_R & 0x10) && sw1 == 0 ){
			...
			sw1 = 1 ;
		}else if(!(GPIO_PORTF_DATA_R & 0x01) && sw2 == 0){
			//Delay100ms(1);
			if(z != 2){
				z = z + 1;
			}
		...
		Nokia5110_PrintBMP(36, arr[z] , vehicle, 0);
		
		
		Nokia5110_DisplayBuffer();     // draw buffer
		Delay100ms(1);              // delay 0.1 sec at 50 MHz
		
		if( ( border >= 15 && border <= 39 )&& z == j){
			break;
		}else {
			score++;
		}
